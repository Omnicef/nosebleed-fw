...
- **Boot was intermittently silent (~1 in 3 resets)** — the chip boots but produces no USB output until another RESET. Originally attributed to USB-CDC re-enumeration. **Resolved 2026-09-16: the real cause was the WiFi modem-sleep PHY wake, which aborts on `phy_track_pll_init` `ESP_ERR_NO_MEM` under load (see WORKLOG T-5.7 boot-loop diagnosis); `WiFi.setSleep(false)` fixed it.** Post-fix survey: 18/18 EN resets produced full boot + app output (0 silent vs ~1-in-3 before; p=0.04% if the old rate still applied). Caveat: the Phase 0 flake was observed across bring-up firmware including WiFi-less builds, so re-enumeration may have contributed to some early instances — but none reproduce now.
